...
class PMPieces {
  ...
  static PMPieces breakIntoPieces(Path path, double precision) {
    ...
    metrics.forEach((metric) {
      for (var i = 0.0; i < 1.11; i += precision) {
        cPath.addPath(
            metric.extractPath(
                metric.length * (i - precision), metric.length * i),
            Offset.zero);
        paths.add(Path()..addPath(cPath, Offset.zero));
        final offsetPoint =Offset((metric.getTangentForOffset(metric.length * i)?.position.dx??0), metric.getTangentForOffset(metric.length * i)?.position.dy??0) ;
